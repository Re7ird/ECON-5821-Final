...
data_1=data_1 |> mutate(inflation=log(CPI)-log(lag(CPI,n=12)))|> filter(is.na(inflation)==F)
data_1=data_1 |> mutate(y=lead(inflation))|> filter(is.na(y)==F)|> select(!c("CPI"))

#Lag=2:
data_2=merge(cpi,X,by="month",all = T)
data_2=data_2 |> mutate(inflation=log(CPI)-log(lag(CPI,n=12)))|> filter(is.na(inflation)==F)
data_2=data_2 |> mutate(y=lead(inflation,n=2))|> filter(is.na(y)==F)|> select(!c("CPI"))

#Lag=3:
data_3=merge(cpi,X,by="month",all = T)
data_3=data_3 |> mutate(inflation=log(CPI)-log(lag(CPI,n=12)))|> filter(is.na(inflation)==F)
data_3=data_3 |> mutate(y=lead(inflation,n=3))|> filter(is.na(y)==F)|> select(!c("CPI"))
...
dim=8
res <-  foreach (l = c(12,60),.combine="cbind") %do%{
  runtest(wdl = l)
}
